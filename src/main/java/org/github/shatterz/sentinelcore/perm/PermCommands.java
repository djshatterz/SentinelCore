                                ...
                                argument("node", string())
                                    .executes(
                                        ctx -> {
                                          ...
                                          if (player == null) {
                                            src.sendFeedback(
                                                () -> Text.literal("Player-only for now."), false);
                                            return 1;
                                          }
                                          ...
                                          src.sendFeedback(
                                              () -> Text.literal("perm(" + node + ") = " + ok),
                                              false);
                                          return ok ? 1 : 0;
                                        })))
                        // /sccore perm role <player> <role>git
                        .then(
                                literal("role")
                                                                        ...
                                                                        .executes(ctx -> {
                                                                            ...
                                                                            if (ok) {
                                                                                src.sendFeedback(() -> Text.literal(
                                                                                        "Assigned role '" + role + "' to " + target.getName().getString() + "."), true);
                                                                                return 1;
                                                                            } else {
                                                                                src.sendError(Text.literal("Failed to assign role. See console."));
                                                                                return 0;
                                                                            }
                                                                        }))))
            )
                            ...
                            .executes(
                                ctx -> {
                                  ...
                                  if (ok) {
                                    ctx.getSource()
                                        .sendFeedback(
                                            () -> Text.literal("SentinelCore config reloaded."),
                                            true);
                                    return 1;
                                  } else {
                                    ctx.getSource()
                                        .sendError(
                                            Text.literal(
                                                "Reload failed. Check console for details."));
                                    return 0;
                                  }
                                })));
